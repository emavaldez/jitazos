...
export default function Home() {
  ...
  return (
    <>
      {/* Animación pulsante verde-negro para la carta activa */}
      <style>{`
        @keyframes cardPulse {
      ...
      <main className="min-h-screen bg-black text-white p-4 font-sans pb-56">
        ...
        <div className="flex justify-around items-center mb-10 bg-zinc-900/50 p-4 rounded-3xl border border-zinc-800 backdrop-blur-md sticky top-4 z-50">
          ...
          <button onClick={resetGame} className="text-2xl font-black italic text-green-500 tracking-tighter hover:scale-105 transition-all active:scale-95" title="Volver al inicio">
            HITAZOS
          </button>
        ...
        <div className="flex flex-wrap justify-center gap-2 items-center max-w-7xl mx-auto px-4 mb-8">
          <DropZone index={0} active={dragOverIndex === 0} onDragOver={() => setDragOverIndex(0)} onDragLeave={() => setDragOverIndex(null)} onDrop={() => handleDrop(0)} onClick={() => confirmarYColocar(0)} />
          {timelineActiva.map((song, i) => (
            <div key={song.id + i} className="flex items-center gap-2">
              ...
              <DropZone index={i + 1} active={dragOverIndex === i + 1} onDragOver={() => setDragOverIndex(i + 1)} onDragLeave={() => setDragOverIndex(null)} onDrop={() => handleDrop(i + 1)} onClick={() => confirmarYColocar(i + 1)} />
            </div>
          ))}
        </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 space-y-2">
                  <input
                    ...
                    className="w-full bg-zinc-900 border border-zinc-700 p-3 rounded-xl outline-none focus:ring-2 ring-green-500 text-xs"
                  />
                  <input
                    value={artistGuess}
  ...
  return (
    <button
      ...
      className={`w-8 h-36 rounded-xl border-2 border-dashed transition-all flex items-center justify-center font-black text-xl active:scale-95 ${
        active ? "border-green-400 bg-green-400/20 text-green-400 scale-110" : "border-zinc-700 hover:border-green-500 hover:bg-green-500/10 text-zinc-600 hover:text-green-500"
      }`}
    >
      +
    </button>
